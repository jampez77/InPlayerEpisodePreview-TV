...
| `?native-playback=1` | Simulate Jellyfin's local playback action, including remote OK and pointer activation. Combine with `&media=movie` or `&media=live-tv`; without this option the demo exercises the session-command fallback. |
